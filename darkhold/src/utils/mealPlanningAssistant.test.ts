...
describe('mealPlanningAssistant', () => {
  ...
  it('does not add a due-again score when a recipe has no history', () => {
    ...
    expect(plan.slots[0]?.selected.components.some((component) => component.key === 'due-again')).toBe(
      false,
    );
  });
  ...
  it('adds a due-again score for recipes overdue against regular monthly cadence', () => {
    const overdueRecipe = makeRecipe(1, 'Zesty Monthly Curry');
    ...
    expect(plan.slots[0]?.selected.recipe.id).toBe(overdueRecipe.id);
    const dueComponent = plan.slots[0]?.selected.components.find(
