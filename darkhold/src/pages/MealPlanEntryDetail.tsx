...
export function MealPlanEntryDetail() {
  ...
  const { data: entry, isLoading: entryLoading, isError: entryError } = useQuery({
    queryKey: ['meal-plan-entry', entryId],
    queryFn: () => apiGet<MealPlan>(`/meal-plan/${entryId}/`),
    enabled: !!entryId,
  });

  const recipeId = entry ? (typeof entry.recipe === 'object' ? entry.recipe.id : entry.recipe) : undefined;

  const { data: recipe, isLoading: recipeLoading, isError: recipeError } = useQuery({
  ...
  return (
    <>
      <div className="mb-2">
        <Link to="/meal-plan" className="small text-muted">← Back to meal plan</Link>
      </div>
      <RecipeDetailContent
        recipe={recipe}
